...
| Weakness | Fix |
|----------|-----|
| **Trafilatura extraction blocked the event loop** | `build_cleaned_text()` is now `async` and calls `extract_clean_text()` via `await asyncio.to_thread(...)` (`main.py:313`, `:345`) - same pattern already used for the Groq call. Both `/explain` and `/explain-stream` now `await build_cleaned_text(request)`. |
| **`/explain-stream` had zero rate-limit resilience** | It previously called Groq directly with no retry logic (only `/explain` had it). Both endpoints now go through `_call_groq_with_fallback()` (`main.py:444`), which reuses `_call_groq_with_retry()`'s exponential backoff. |
| **`FALLBACK_MODEL` and `redis` were declared but unused** | `FALLBACK_MODEL` is now wired via `_call_groq_with_fallback()` (`main.py:444`): if `PRIMARY_MODEL` is still rate-limited past its retry budget or hard-errors, it falls back once to `FALLBACK_MODEL`. The unused `redis==5.0.1` dependency was removed from `requirements.txt` entirely (actually integrating a real Redis-backed cache would require provisioning external infra - an Upstash/Redis instance and `REDIS_URL` - which is a deployment decision, not a code fix, so it was deleted rather than left as dead weight). |
| **No size cap on `text`, `question`, or `title`** | `MAX_HTML_LENGTH` used to only guard `content_type == "html"` payloads; plain text, `question`, and `title` had no limit. `ExplainRequest` now has `Field(max_length=...)` on all three (`main.py:233-243`): `text` capped at `MAX_HTML_LENGTH` (2,000,000, regardless of `content_type`), `question` at the new `MAX_QUESTION_LENGTH` (2,000), `title` at the new `MAX_TITLE_LENGTH` (500). Enforced by Pydantic before the route handler runs, so the old manual `413` check in `build_cleaned_text()` (which would now be unreachable dead code) was removed - an over-limit request gets a generic `422` instead. |
| **Loading spinner never reset between questions** | `hasStoppedLoading` (`popup.ts`) was set `true` once `stopLoading()` ran and never reset, so only the 1st question in a session ever got the loading animation cleared on its first response chunk - every question after that kept "Tea is boiling…" on screen for the whole stream. Fixed by resetting `hasStoppedLoading = false` at the top of `showLoading()` (`popup.ts:326`), which runs at the start of every `submit()`. |
| **Wider data-exposure surface (partially mitigated)** | `getRenderedHTML()` (`content.ts:29`) now calls `stripSensitiveContent()` before serializing to `outerHTML`: removes password fields and `input[type="hidden"]` (CSRF/session tokens) entirely, clears `<input>`/`<textarea>` values and `<option selected>` (form values can be live-reflected into these attributes by frameworks like React), and removes elements hidden via `[hidden]`, `[aria-hidden="true"]`, inline `display:none`/`visibility:hidden`, or `<template>`. **This narrows, but does not close,** the exposure gap: the pipeline still sends the entire *visible* rendered DOM (not just article text) to the backend/Groq, and CSS-class-based hiding (external stylesheet rules, as opposed to an inline `style` attribute) isn't detected. |
| **Empty/blank error messages in server logs and the streamed `ERROR:` payload** | `str(e)` is an empty string for several real exception types (e.g. some `httpx` transport errors raised with no message), so `print(f"❌ Stream error: {e}")` and the SSE `ERROR: {e}` sent to the popup could both render completely blank - undiagnosable in server logs and confusing in the UI. Both `explain_stream()`'s and `get_groq_response()`'s generic exception handlers now always include the exception's type name and fall back to `"(no message)"`, and log a full `traceback.print_exc()` server-side (never sent to the client). |
| **CORS is not authentication - no auth or rate limiting on the backend** | Anyone who found the backend URL could call `/explain-stream` directly with `curl`, bypassing CORS entirely, with no limit on how many requests they could send. Added `verify_api_key()` (`main.py:103`, checks an `X-API-Key` header against `BACKEND_API_KEY`) and a dependency-free in-memory per-IP `rate_limiter()` (`main.py:141`), both applied to `/explain`, `/explain-stream`, and `/normalize-query`. The matching header is sent from `frontend/src/background.ts`'s `backendHeaders()`. **Honest caveat:** the shared secret ships inside the built extension's JS - anyone who unpacks the `.crx`/`.zip` can read it out. This isn't real secrecy against a determined attacker; it raises the bar from "just know the URL" to "inspect the extension bundle first," which is a meaningful improvement for a personal/local project but not a substitute for real per-user auth if this is ever opened up to multiple untrusted users. Both `BACKEND_API_KEY` (backend) and the matching constant (frontend) default to empty, so this fails open with a startup warning until explicitly configured on both sides. |
| **`/docs`, `/redoc`, `/openapi.json` publicly browsable with zero auth** | FastAPI's default `docs_url`/`redoc_url`/`openapi_url` left the full API schema browsable, with Swagger's "Try it out" letting anyone fire real requests without ever reading the source. Now disabled by default (`main.py:23-34`: `docs_url`/`redoc_url`/`openapi_url` are all `None` unless `ENABLE_API_DOCS=true` is set in `.env`), so all three routes 404 like any other unknown path. Set `ENABLE_API_DOCS=true` for local development only - never on a public deployment. |
